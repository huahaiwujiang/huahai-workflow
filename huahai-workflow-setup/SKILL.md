...
| grill-me | 需求追问 | `npx skills@latest add mattpocock/skills -g -y -s grill-me -s grill-with-docs` |
| grill-with-docs | 文档审查 | 同上 |

## 🔴 CHECKPOINT 安装后验证

安装完成后，🛑 STOP 必须先验证再继续：

1. 重新运行 `node .claude/skills/huahai-workflow-setup/scripts/check-deps.mjs`
2. 确认输出 `✅ 所有依赖已就绪`
