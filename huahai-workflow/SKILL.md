...
description: 开发工作流——从需求追问到代码发布。当用户提出开发任务、新功能、/huahai-workflow 时，必须使用此skill来编排工作流程。内置gf git流程管理、CHECKPOINT门禁、todolist进度追踪。
---

# 开发工作流

> superpowers 管思考，Matt Pocock 管追问，内置命令管审查。

## 🧭 启动决策树（读到本 skill 后第一件事）

```
读 todolist.md
├─ 不存在 ──→ 删除 plan/requirements.md（如有），从追问开始（不创建 todolist）
├─ 存在 + 有未勾选任务 ──→ 跳到对应步骤继续
└─ 存在 + 全部已勾选
   ├─ 已发布（git log origin/main..HEAD 为空，无未推送提交）──→ 删除 todolist.md + plan/requirements.md，问"新功能？"→ 追问
   └─ 未发布 ──→ 提示用户先执行发布步骤
```
...
步骤1: 追问（WHAT — 用户要什么）
  ...
  输入: 用户提供的需求文档/描述
  输出: 澄清后的需求理解，纯业务视角——用户故事、验收条件、业务规则、范围边界
  规则: 只读项目文档，不看代码。🚫 禁止输出 API 路径、表结构、技术栈、架构图
  完成: 用户确认需求无误 + 需求理解写入 plan/requirements.md（落盘，设计阶段从此读入）

步骤2: 设计（HOW — 怎么实现）
  🔴 强制: Skill("superpowers:brainstorming") — 加载后严格遵循其指令
  输入: plan/requirements.md（追问阶段落盘的需求文档）
  输出: 技术方案——API 设计、数据库 schema、技术选型、架构图，写入 plan/ 目录
...
## plan/requirements.md 规约

- **创建时机**：追问完成后，将确认的需求写入
- **格式**：纯业务视角——用户故事、验收条件、业务规则、范围边界
- **生命周期**：随新工作流启动或发布完成而删除（与 todolist.md 同步清理）
- **gitignore**：`plan/` 目录应加入 `.gitignore`

## todolist.md 规约

- **创建时机**：唯一在拆分完成后，从 writing-plans 的计划中提取可勾选任务
- **顶部元信息**：必须记录拆分阶段询问的输出路径，格式：
  ```
  <!-- plan: <绝对或相对路径> -->
  ```
- **格式**：每行 `- [ ] 任务描述`，已完成改为 `- [x]`
- **粒度**：一项 = 一个可独立验证的原子任务
- **生命周期**：所有任务 ✅ + 发布完成 → 删除（同时删除 plan/requirements.md）
- **gitignore**：创建时确认已加入 `.gitignore`
- **损坏处理**：格式无法解析时，根据 git log 和 plan 目录推断当前进度，从推断的步骤重新开始

## 会话恢复

1. 读 `todolist.md`
2. 不存在 → 删除 `plan/requirements.md`（如有），追问
3. 存在 + 有 `- [ ]` → 对应步骤继续
4. 存在 + 全 `- [x]` → 检查发布状态 → 删除 todolist.md + plan/requirements.md 或提示
...
| "这步已经做过了" | 验证产物是否存在（如方案文档、plan 目录），存在即可跳过 |
| "换个方案" | 回到设计，重新 brainstorm |
...
| 2. 设计 | 方案分歧大 | 列2个对比方案优劣，让用户选 | 用户不选 → 默认简单方案，记录理由到 todolist.md |
| 3. 拆分 | 任务粒度过大 | 继续拆到每项 ≤5 分钟、单文件可完成 | 仍过大 → 标记需人工拆分 |
...
| 6 | 追问阶段就搜索/阅读代码库 | 先理解文档，后看代码。追问阶段只读文档不看代码 |
