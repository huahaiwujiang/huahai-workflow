---
name: seven-step
description: 6步开发工作流——从需求追问到代码发布。当用户提出开发任务、新功能、/seven-step 时，必须使用此skill来编排工作流程。内置gf git流程管理、CHECKPOINT门禁、todolist进度追踪。
---

# 6 步工作流

> superpowers 管思考，Matt Pocock 管追问，内置命令管审查。

## 🧭 启动决策树（读到本 skill 后第一件事）

```
读 todolist.md
├─ 不存在 ──→ 从步骤1开始（不创建 todolist）
├─ 存在 + 有未勾选任务 ──→ 跳到对应步骤继续
└─ 存在 + 全部已勾选
   ├─ 已发布（git log origin/main..HEAD 为空，无未推送提交）──→ 删除 todolist.md，问"新功能？"→ 步骤1
   └─ 未发布 ──→ 提示用户先执行步骤6发布
```

## 6 步链路

每一步都有明确的 **输入 → 工具 → 输出 → 完成标志**。

```
步骤1: 追问（WHAT — 用户要什么）
  🔴 强制: Skill("grill-with-docs") — 加载后严格遵循其指令
  输入: 用户提供的需求文档/描述
  输出: 澄清后的需求理解，纯业务视角——用户故事、验收条件、业务规则、范围边界
  规则: 只读项目文档，不看代码。🚫 禁止输出 API 路径、表结构、技术栈、架构图
  完成: 用户确认需求无误 + 需求理解写入 plan/requirements.md（落盘，步骤2从此读入）

步骤2: 设计（HOW — 怎么实现）
  🔴 强制: Skill("superpowers:brainstorming") — 加载后严格遵循其指令
  输入: plan/requirements.md（步骤1 落盘的需求文档）
  输出: 技术方案——API 设计、数据库 schema、技术选型、架构图，写入 plan/ 目录
  完成: 🔴 CHECKPOINT — 展示方案，等待用户确认

步骤3: 拆分
  🔴 强制: Skill("superpowers:writing-plans") — 加载后严格遵循其指令
  输入: 确认后的方案
  规则: 开始前先问用户「实施计划输出到哪个目录？」（默认 /plan），记录到 todolist.md 顶部
  输出: <用户指定目录>/ 下的实施计划 + todolist.md（从计划中提取可勾选清单）
  完成: todolist.md 已创建，每项是原子化任务

步骤4: 编码
  🔴 强制: 先判断任务类型——纯配置/文档/typo 修复直接改；其余代码任务必须 Skill("superpowers:test-driven-development")，走 RED → GREEN → REFACTOR
  输入: todolist.md 中的任务
  完成: 所有任务 ✅，代码类任务测试全绿

步骤5: 审查
  🔴 强制（两步都要跑，不可跳过）:
    1. Skill("code-review") — diff 审查，找 correctness bugs + reuse/simplification/efficiency
    2. Skill("security-review") — 安全审查
  输入: 步骤4的代码变更
  完成: 🔴 CHECKPOINT — 检查清单全通过

步骤6: 发布
  🔴 强制: Skill("gf") 完成 git 流程
  输入: 审查通过的代码
  完成: 代码已推送
```

### 每一步完成后

1. 勾选 `todolist.md` 中对应任务
2. 确认完成标志已达成
3. 进入下一步

## 🔴 CHECKPOINT 门禁

### 步骤2→3：人类确认（铁律）
方案设计完成后，向用户展示方案并等待明确确认。未确认 = 🛑 STOP。

### 步骤5→6：提交前检查清单
- [ ] TDD 测试全部通过
- [ ] Skill("code-review") 通过，无 CRITICAL 问题
- [ ] Skill("security-review") 通过（涉及 auth/finance/system 时强制）
- [ ] 文档变更已同步
- [ ] todolist.md 对应任务全部 ✅

全部通过 → 提交。任一未通过 → 🛑 STOP，先补。

## todolist.md 规约

- **创建时机**：唯一在步骤3完成后，从 writing-plans 的计划中提取可勾选任务
- **顶部元信息**：必须记录步骤3 询问的输出路径，格式：
  ```
  <!-- plan: <绝对或相对路径> -->
  ```
- **格式**：每行 `- [ ] 任务描述`，已完成改为 `- [x]`
- **粒度**：一项 = 一个可独立验证的原子任务
- **生命周期**：所有任务 ✅ + 步骤6发布完成 → 删除
- **gitignore**：创建时确认已加入 `.gitignore`
- **损坏处理**：格式无法解析时，根据 git log 和 plan 目录推断当前进度，从推断的步骤重新开始

## 会话恢复

1. 读 `todolist.md`
2. 不存在 → 步骤1
3. 存在 + 有 `- [ ]` → 对应步骤继续
4. 存在 + 全 `- [x]` → 检查发布状态 → 删除或提示

## 用户绕路处理

用户可能说"直接写代码"、"跳过设计"、"这步不用做了"。处理原则：

| 用户意图 | 处理 |
|---------|------|
| "跳过步骤X" | 说明该步为什么不可跳过（一句话），若用户坚持则记录到 todolist.md 顶部注释 |
| "这步已经做过了" | 验证产物是否存在（如方案文档、plan 目录），存在即可跳过 |
| "换个方案" | 回到步骤2，重新 brainstorm |
| "需求变了" | 回到步骤1，追加 todolist.md 顶部注释说明变更 |
| 步骤中被打断 | 下次会话通过 todolist.md 恢复，从当前步骤继续 |

## 失败处理

| 步骤 | 触发条件 | 一线修复 | 仍失败兜底 |
|------|---------|---------|-----------|
| 1. 追问 | 需求模糊 | 追问直到清晰 | 超过3轮无结论 → 列出待澄清项让用户逐条确认 |
| 2. 设计 | 方案分歧大 | 列2个对比方案优劣，让用户选 | 用户不选 → 默认简单方案，记录理由到 todolist.md |
| 3. 拆分 | 任务粒度过大 | 继续拆到每项 ≤5 分钟、单文件可完成 | 仍过大 → 标记需人工拆分 |
| 4. 编码 | 测试写不出 | 先写最小实现让测试通过 | 再逐步加边界，每加一个跑全量；非代码任务跳过 |
| 5. 审查 | Skill("code-review") 发现严重问题 | 修复后重跑 Skill("code-review") | 最多循环 2 次，仍不过 → 标记已知风险，用户决策 |
| 6. 发布 | gf 合并冲突 | 显示冲突文件，等待用户手动解决 | 解决失败 → `git merge --abort`，记录冲突到 todolist.md |

## 🔴 反例（绝不）

| # | 反模式 | 原因 |
|---|--------|------|
| 1 | 跳过人类确认直接写代码 | 方向可能全错 |
| 2 | 不写测试直接写实现 | 没测试 = 不可信 |
| 3 | 步骤未完成就跳下一步 | 技术债叠加 |
| 4 | Skill("code-review") 有 CRITICAL 仍 commit | 生产埋雷 |
| 5 | 跳过步骤1-3 直接写代码 | 需求不清 = 必返工 |
| 6 | 步骤1就搜索/阅读代码库 | 先理解文档，后看代码。步骤1只读文档不看代码 |
